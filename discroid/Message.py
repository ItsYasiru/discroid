...
class Message:
    def __init__(self, data: dict):
        ...
        self.edited_timestamp: Optional[str] = (
            _edited_timestamp if (_edited_timestamp := data.get("edited_timestamp")) else None
        )
        ...
        self.mention_roles: list[Role] = [
            Role(_role) for _role in data.get("mention_roles", list())
        ]
        self.mention_channels: list[ChannelMention] = [
            ChannelMention(_mention) for _mention in data.get("mention_channels", list())
        ]
        self.mention_everyone: bool = data.get("mention_everyone", False)

        self.embeds: list[Embed] = [Embed(_embed) for _embed in data.get("embeds", list())]
        self.reactions: list[Reaction] = [
            Reaction(_reaction) for _reaction in data.get("reactions", list())
        ]
